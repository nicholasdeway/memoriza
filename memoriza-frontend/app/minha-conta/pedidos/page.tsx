...
export default function PedidosPage() {
  ...
  const userOrders = mockOrders.filter((order) => order.clienteId === user?.id || order.clienteId === "2")

  const filteredOrders = userOrders.filter((order) => {
    const matchesSearch =
      order.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
      order.items.some((item) => item.produtoNome.toLowerCase().includes(searchTerm.toLowerCase()))
    ...
    <div className="bg-background border border-border rounded-xl p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-foreground/60">Acompanhe seus pedidos e histórico de compras</p>
          {statusFilter && (
            <div className="flex items-center gap-2 mt-2">
              <span className="text-sm bg-accent/10 text-accent px-2 py-1 rounded-md">
                Filtro: {orderStatusLabels[statusFilter as keyof typeof orderStatusLabels]}
              </span>
              <button onClick={clearFilter} className="text-foreground/60 hover:text-foreground">
                <X size={14} />
              </button>
            </div>
          )}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  {order.status === "a_caminho" && (order as any).codigoRastreio && (
                    <div className="mt-2 text-xs text-accent">
                      <p className="font-medium">Código: {(order as any).codigoRastreio}</p>
                      <p className="text-foreground/60">{(order as any).transportadora}</p>
                    </div>
                  )}
                </div>
              </div>
